...
func (client *deepSeekClient) complete(messages []message) (completion, error) {
	payload := chatRequest{Model: "deepseek-chat", Messages: messages, Stream: false}
	...
	if response.StatusCode < 200 || response.StatusCode >= 300 {
		if parsed.Error != nil {
			return completion{}, fmt.Errorf("deepseek api: status=%d type=%s message=%s", response.StatusCode, parsed.Error.Type, parsed.Error.Message)
		}
