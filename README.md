...
[LLM Wiki](wiki/index.md) содержит карту архитектуры, инструкции запуска,
правила памяти и tool calling, состояние локальной работы и маршрут восстановления
контекста после новой сессии. Точка входа для coding-агента: [AGENTS.md](AGENTS.md).

Wiki ведётся через skill `karpathy-llm-wiki`: исходные снимки в `raw/`, статьи
в `wiki/`, журнал операций в `wiki/log.md`. Прежняя `docs/llm-wiki` сохранена
как историческая документация и больше не является актуальной точкой входа.
